...
架构按完整产品设计（spec v0.3），按垂直切片交付。当前状态：**K1 知识冷启动完成**——全源导入管道实测跑通、题库持续增长（含 LeetCode Hot 100 手撕题）、厂商分类管道上线；下一阶段 I1 面试循环。
...
| `start.bat` | 启动全部：docker 基础设施 → api → agents → web；端口占用探测+自动顺延、健康检查、日志落 `logs/` |
