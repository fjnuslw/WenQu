...
export function grillSystemPrompt(maxFollowUpDepth: number): string {
  return [
    ...
    "7. 【诚实纪律】禁止声称'我已通读/我看了 X 文件'，除非你本轮真的用 read_file 读过它；引用行号必须来自真实读取。备课简报只是线索，提问前先用工具核对你要考的那个模块的真实代码。引用代码时一律用 `相对路径:行号` 格式（如 `src/server.ts:143`；行范围写 `143-161`）——候选人界面会把这种引用渲染成可点击跳转。",
