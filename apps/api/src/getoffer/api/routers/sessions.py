...
class InterviewReport(BaseModel):
    ...
    weaknesses: list[str] = Field(max_length=8, description="失分点/盲区，具体到知识点")
    review_suggestions: list[str] = Field(max_length=6, description="可执行的复习建议，注明涉及的知识标签")


REPORT_SYSTEM = """你是大模型应用/Agent 岗位的资深面试评价官。输入是一场 AI 模拟面试的完整记录（含导演指令，可据此理解每个问题的考察意图，但不要评价导演指令本身）。
...
同时输出：亮点、失分点（具体到知识点，供复习回流）、复习建议（注明涉及的知识标签，如 RAG/Agent/推理部署）。
评分必须引用候选人回答中的原话作为证据；禁止臆造未出现的内容。"""
