...
| **G1 项目拷打** | 备课流水线（repomap 移植/cAST/pgvector/wiki/git 归属）、疑点映射、只读工具面、拷打 agent、证据链报告 | 真实 repo 全流程达 §4 验收 | 🔄 **v1 全链路已落地；v2 代码已收口，仅 Provider gate 待验（2026-09-02）**：Tree-sitter repo map、370 个真实代码块的 pgvector 接线、公共 Git URL、200-commit 归属分析、动态只读工具和 Web 状态均已真实运行；当前 embedding Provider=disabled，故不把 fixture 向量 Top-5 冒充语义质量。证据与唯一剩余门见 [IMP-G1-001](./improvement-specs/IMP-G1-001-repository-intelligence-v2.md)。原条目的 Anki 导出已由 F6 通用导出实现，不重复建设。 |
