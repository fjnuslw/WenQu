...
| `manual-douyin` | 抖音人工复制粘贴 / 授权后的可见浏览器逐帖识图 | 后端不访问原站；当前浏览器会话需用户完成抖音登录 | N/A | 通道与 UI 已完成；登录后可采集 |
...
| 抖音人工摘录 | 0 | 0 | 0 | 0 | 独立渠道和 UI 入口已完成；当前浏览器搜索页仍显示登录墙，未伪造或绕过登录态 |
| 朋友分享广告样例 | 0 | 0 | 0 | 0 | 明确资料领取/引流文本，`skipped_non_experience=1` |

总计 43 条面经。CSDN 三篇都触及单帖 schema 的 30 题上限；浏览器辅助导入的面壁智能 9 图长帖同样触及上限（原帖 57 个提问点，人工转录其中 48 个技术相关提问，当前 schema 忠实保留前 30 个），新增淘天一面也达到 30 题上限。没有把同一场面试静默拆成多条记录。若后续要完整容纳超长帖或真正的“多家公司合一”长帖，应显式升级批量/分段模型并设计同场聚合语义。
...
浏览器实测 `/experiences`：显示来源 tab `牛客 16 / CSDN 3 / 小红书人工摘录 24`；点击“人工摘录”可见文本、五渠道（含“抖音人工摘录”）、日期、URL 字段和“不访问原帖”说明；页面显示 `43/43 条面经`，新卡片均带来源和原帖链接。百度 Agent Harness 三轮面正常展开 28 题，淘天卡片正常展开 30 题。
...
- 抖音当前浏览器标签页未登录；用户完成扫码登录后可继续低频逐帖审核，不应把登录墙当成可绕过的技术故障。
