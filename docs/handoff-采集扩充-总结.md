...
| `manual-douyin` | 抖音人工复制粘贴 / 授权后的可见浏览器逐帖识图 | 后端不访问原站；用户登录后仅审核可见页面 | N/A | 可用；已完成 9 篇真实面经入库 |
...
未接入知乎自动采集：对公开回答 URL 走 `PoliteClient` 实测，直接得到 `compliance_violation`，robots 禁止该路径。实现阶段没有编写或提交任何小红书/抖音自动采集代码；后续用户明确授权后，使用已登录的可见浏览器低频逐帖阅读与翻图识别，内容分别经 `manual-xhs` / `manual-douyin` 人工端点入库，不调用隐藏接口、不提取 Cookie、不做无人值守翻页。
...
| 抖音人工摘录 | 9 | 4/9（44.4%） | 114 | 0 | 覆盖腾讯、美团、字节、米哈游、联想、大疆、哔哩哔哩、美的和真实小公司 Agent 岗；逐帧读取滚动录屏与图文 |
| 朋友分享广告样例 | 0 | 0 | 0 | 0 | 明确资料领取/引流文本，`skipped_non_experience=1` |

总计 52 条面经。CSDN 三篇都触及单帖 schema 的 30 题上限；浏览器辅助导入的面壁智能 9 图长帖同样触及上限（原帖 57 个提问点，人工转录其中 48 个技术相关提问，当前 schema 忠实保留前 30 个），新增淘天一面也达到 30 题上限。没有把同一场面试静默拆成多条记录。若后续要完整容纳超长帖或真正的“多家公司合一”长帖，应显式升级批量/分段模型并设计同场聚合语义。
...
浏览器实测 `/experiences`：显示来源 tab `牛客 16 / CSDN 3 / 小红书人工摘录 24 / 抖音人工摘录 9`；点击“人工摘录”可见文本、五渠道（含“抖音人工摘录”）、日期、URL 字段和“不访问原帖”说明；页面显示 `52/52 条面经`，新卡片均带来源和原帖链接。腾讯抖音面经正常展开 27 题，哔哩哔哩抖音面经正常展开 13 题。
