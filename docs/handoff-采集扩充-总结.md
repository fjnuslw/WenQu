...
> 完成日期：2026-08-30
> 范围：牛客多话题、CSDN 公开页、linux.do 合规尝试、小红书/抖音/知乎/Reddit 等人工摘录通道。
...
| `manual-zhihu` | 知乎人工复制粘贴 / 授权后的可见浏览器阅读 | 后端不访问原站；用户自行登录后仅审核可见文章 | N/A | 可用；已完成 25 篇真实面经入库 |
| `manual-reddit` | Reddit 人工摘录 | 后端不访问原站；仅整理可见的个人面试原帖 | N/A | 可用；已完成 2 篇真实面经入库 |
| `manual-linux-do` | linux.do 人工摘录 | 后端不访问原站；自动 RSS/JSON 与可见搜索均受限时不绕过 | N/A | 通道可用；本轮无满足质量门槛的新帖 |
...
|---|---:|---:|---:|---:|---|
| 牛客 | 30 | 18/30（60.0%） | 410 | 11 | 30 帖扩充批次新增 14、跳过 7 篇资料/非面经；覆盖字节 AI Agent、项目深挖等，词表外公司只报 unmatched |
...
| 知乎人工摘录 | 25 | 10/25（40.0%） | 246 | 0 | 新增个人长文中的 17 个公司面试章节；按公司显式拆分，未把简历挂或无问题章节计为面经 |
| Reddit 人工摘录 | 2 | 0/2（0%） | 21 | 0 | SarvamAI ML Engineer 与 Cloudflare AI Engineer 两篇个人经历；公司均不在当前词表，保留 unmatched |
| 朋友分享广告样例 | 0 | 0 | 0 | 0 | 明确资料领取/引流文本，`skipped_non_experience=1` |

总计 106 条面经、1443 个主问题、12 个追问链节点。CSDN 三篇都触及单帖 schema 的 30 题上限；浏览器辅助导入的面壁智能 9 图长帖同样触及上限。知乎新增长文是作者亲历多家公司、且正文逐公司成章，因此按公司明确拆成 17 条独立面经；同一公司的多轮仍保持在一条记录内，没有按轮次或题目拆卡凑数。没有实际问题的简历挂、失约、广告与求助帖继续跳过。
...
### 补足 100 条：知乎长文、Reddit 与牛客增量

```text
知乎多公司个人长文：17 个有实际面试问题的公司章节
首次：17 × inserted=1
整批原样重跑：17 × duplicates=1, inserted=0

Reddit：SarvamAI ML Engineer / Cloudflare AI Engineer
首次：2 × inserted=1
原样重跑：2 × duplicates=1, inserted=0

POST /api/ingest/collect/nowcoder?max_posts=30
{"posts_seen":30,"duplicates":9,"skipped_non_experience":7,
 "inserted":14,"unmatched_companies":["腾讯WXG","名创优品","影石","影石创新"]}

立即重跑
{"posts_seen":30,"duplicates":23,"skipped_non_experience":7,
 "inserted":0,"unmatched_companies":[]}
```

批次前 73 条，知乎新增 17、Reddit 新增 2、牛客新增 14，最终 106 条。知乎长文按作者原有公司章节显式拆分，不拆同一公司轮次；Reddit 跳过站内 AI 摘要，只取发帖人的个人面试正文。linux.do 可见搜索本轮返回“操作次数过多”，叠加已有 RSS/JSON Cloudflare 403，未重试轰炸、未带登录态或绕过挑战；新增 `manual-linux-do` 供后续用户人工提供正文时使用。

面经页原先单次最多加载 100 条；总量超过 100 后改为按 `offset` 继续读取直到 `items.length == total`，避免出现 `100/106` 和来源 tab 少计。API 两页实测合并返回 106 条。
...
生产构建重启后浏览器实测 `/experiences`：`106/106 条面经`；来源 tab 为 `牛客 30 / 知乎 25 / 小红书 24 / 抖音 22 / CSDN 3 / Reddit 2`。人工摘录弹层可见 Reddit 与 linux.do 两个新增选项，Reddit 两张卡片的原帖链接、岗位和 21 个问题均正常渲染。
...
9a46d38 扩充抖音与知乎真实面经
(本次提交) 扩充面经至106条并新增Reddit通道
```

文档续十八与本总结由后续文档提交承载。所有功能提交均在 `main`，提交者沿用仓库配置 `fjnuslw`。

## 7. 2026-08-31：公开目录扩充至 503 条

### 7.1 最终规模

本轮从 106 条继续扩充，最终保留 **503 条真实面经**，新增 397 条；共 **5671 个主问题、74 个追问节点**。统计来自最终质量清理后的数据库，而非候选 URL 数量。

| 来源 | 面经 | 命中公司 | 主问题 | 追问 |
|---|---:|---:|---:|---:|
| 牛客既有通道（`nowcoder`） | 30 | 18 | 410 | 11 |
| CSDN | 11 | 5 | 199 | 0 |
| 小红书人工摘录 | 24 | 12 | 407 | 1 |
| 抖音人工摘录 | 22 | 14 | 269 | 0 |
| 知乎人工摘录 | 25 | 10 | 246 | 0 |
| Reddit 人工摘录 | 9 | 0 | 63 | 0 |
| 牛客公开目录（`nowcoder-public`） | 278 | 160 | 3103 | 60 |
| GitHub EasyOffer | 21 | 20 | 163 | 0 |
| 博客园 | 39 | 22 | 438 | 0 |
| GeeksforGeeks | 27 | 4 | 281 | 2 |
| linux.do 人工摘录 | 17 | 0 | 92 | 0 |
| **合计** | **503** | **265** | **5671** | **74** |

### 7.2 新增来源与合规边界

- `nowcoder-public`：只采 robots 允许的公开 SSR 详情、话题和用户页；间隔 8 秒。目录发现最多 30 页，带原子检查点，可从中断处继续。搜索路径被 robots 拒绝时显式失败，不换隐藏接口。
- `github-easy-offer`：读取固定公开仓库中的真实面经 Markdown，每个文件对应一位候选人的一次多轮经历；间隔 8 秒。
- `cnblogs`：对两篇已人工审核的公开汇总页按原文公司边界拆分，共 39 条；间隔 12 秒。没有公司标题或没有具体问题的段落不入库。
- `geeksforgeeks`：采集人工筛选过的公开 Interview Experience 文章；间隔 12 秒。一个候选人的多轮面试仍是一条记录，同一候选人的两篇连续记录只在有明确证据时合并。
- `csdn`：在原有 3 条基础上扩充公开单篇面经，并仅对原文明确分公司的文章拆分；间隔 12 秒。
- `manual-linux-do`、`manual-reddit`：后端不请求原站，只导入人工核验后的正文与溯源 URL。linux.do 的 Cloudflare、知乎 robots、Reddit 可达性问题均未通过 Cookie、登录态、挑战绕过或隐藏接口规避。
- 小红书、抖音、知乎继续保持“用户登录 + 可见浏览器低频人工摘录”，没有新增无人值守自动采集器。

### 7.3 目录、批量审核与抽取语义

- 新增检查点目录器与最多 30 条/批的审核导入器；失败条目可显式重试，已完成条目和原始快照可恢复，不因单条失败丢失整批进度。
- 批量抽取沿用单条导入的公司匹配、问题树、`content_hash` 和幂等入库语义；只增加 `ExperienceBatch` 结构，不改变原有单条抽取函数。
- 一位候选人、一个岗位、一次多轮流程只形成一条面经；只有原文存在明确公司章节时才拆分。禁止按轮次、问题或图片分页拆记录凑数。
- 每个导入问题必须能在本批原始正文中逐字定位；过滤“项目介绍”“介绍一下自己”等缺少问题语义的孤立标题、截断句和泛主题词。
- 单条仍受 30 个主问题上限约束；超出时保留前 30 个可核验问题，不复制、不改写、不拆成第二条面经。

### 7.4 质量审计

最终审计结果：503 个记录 ID 全部唯一，503 条均至少包含 1 个问题；本轮新增记录不存在跨来源 URL 重复，也没有把无分段的多候选人汇总文误作单场面试。全库只发现 1 组历史遗留候选重复（旧记录 id 7/9），不属于本轮新增数据，未擅自删除。

质量复核共隔离 24 条候选记录：6 条混合多公司记录、15 条无效/泛化/截断记录，以及最终复核发现的 3 条（混合三家公司、语义重复、仅“项目介绍”）。隔离前均写入内部可恢复备份；原始网页快照、候选清单与审核账本位于本地 `data/collection500/`，不提交完整第三方正文。

容易误收但已主动排除的例子：登录墙页面、SEO 题库/资料广告、无具体问题的求职讨论、拒绝转载的帖子、只有 AI 摘要的 Reddit 页面、只有模糊主题而无原句问题的文章。发布日期仅作为来源时间；没有原文明示时不伪造面试发生日期。

### 7.5 验收记录

```text
API / Web 同源分页：total=503，实际加载 503，ID 唯一 503，含问题 503
/experiences：HTTP 200，页面可完整加载超过 100 条的数据
pytest：55 passed
...
四个自动来源均用真实采集端点做过“首次导入 + 原样重放”验证。最终 GeeksforGeeks 端点连续两次重放同一条均返回：

```json
{"channel":"geeksforgeeks","posts_seen":1,"duplicates":1,
 "skipped_non_experience":0,"inserted":0,"unmatched_companies":[]}
```

EasyOffer、博客园、GeeksforGeeks 各以 `max_posts=5` 重放时同样全部命中重复、`inserted=0`；说明重启和重放不会重复增加记录。采集器继续复用 `PoliteClient`，没有添加直连绕过路径。

### 7.6 后续扩充建议

继续扩量时应优先增加“真实候选人 + 明确岗位/公司 + 具体问题”的新目录，不降低上述口径。先跑目录发现，再人工审核候选批次，最后运行质量审计；不要为了更高数字放宽到题库、课程笔记、岗位 JD 或无原文问题的经验总结。公开页面结构变化、robots 规则变化或登录墙应作为显式失败处理。

### 7.7 本轮 Git 提交

```text
25a6d84 新增 EasyOffer 公开面经采集
a2a6b6d 新增博客园公开面经采集
63ab2c3 新增 GeeksforGeeks 面经采集
5bcb9d0 扩充 CSDN 公开面经采集
e3d6d8a 构建公开面经目录与批量审核管道
```

上述提交均位于 `main`，提交者沿用仓库配置 `fjnuslw`。数据快照与隔离备份不进 Git。
